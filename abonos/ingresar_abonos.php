...
<?php   
    ...
    $tab=1;
    if(!(empty($fecha))){
      $consul_fecha="select detalle_movimientos.num_prestamo";
      $consul_fecha.=" FROM detalle_movimientos, prestamos where detalle_movimientos.num_prestamo = prestamos.num_prestamo";
      $consul_fecha.=" and detalle_movimientos.fecha_m='$fecha' and prestamos.cod_cartera='$cod_cart';";
      $resultado2=mysql_query($consul_fecha,$conex); 
      $m_abo=mysql_fetch_array($resultado2);   
        ...
        $x=0;
        do{
            if(!(in_array('00010333', $m_abo))){
              echo"<tr>";
              echo"<td>".$m_abo['num_prestamo']['$x']."</td>";
