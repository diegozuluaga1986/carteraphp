<?php  
    ...
    if($cartera!="")
    {
        ...
        //$fpdf->SetTextColor(41, 43, 0);
        $fpdf->Cell(50, 10, $mat['nombre_car'], 0);
        $fpdf->Cell(75, 10, $dia, 0);
        $fpdf->Cell(13, 10, 'Pag. ', 0);
        $fpdf->Cell(10, 10, $pag, 0);
        $fpdf->Ln();
        //Consulta prestamos activos
        ...
        // consulta volados
        $consulta_v="select prestamos.num_prestamo, prestamos.num_cliente,";
        $consulta_v.=" prestamos.fecha_inicio,"; 
        $consulta_v.=" DATE_ADD(prestamos.fecha_inicio, INTERVAL prestamos.num_cuotas DAY) 'fecha_fin',";    
        $consulta_v.=" prestamos.monto,";
        $consulta_v.=" prestamos.monto_final,";
        $consulta_v.=" prestamos.cuota,";
        $consulta_v.=" prestamos.num_cuotas, prestamos.orden_ruta, prestamos.cod_cartera, prestamos.dia_abona,"; 
        $consulta_v.=" clientes.nombre, clientes.profesion, clientes.direccion, clientes.barrio,"; 
        $consulta_v.=" clientes.ciudad, clientes.celular, clientes.tel_fijo from";
        $consulta_v.=" prestamos, clientes where prestamos.num_cliente=clientes.num_cliente";
        $consulta_v.=" and prestamos.cod_cartera=$cod_cart and prestamos.estado='3'";
        $consulta_v.=" order by orden_ruta;";
        $resultado_v=mysql_query($consulta_v,$conex);
        $matriz_volados=@mysql_fetch_array($resultado_v); 
        //Fila encabezados
        ...
        {   
            ...
            do
            {
                $p_v=$matriz_volados['num_prestamo'];
                $x=$x+1;
                $suma=0; 
                $atr=0; 
                $cuotas=0;
                ...
                $notas=" ";
                //Buscarmos cuotas asociadas a cada prestamo
                $detalle="select * from detalle_movimientos where num_prestamo=$p_v order by fecha_m;";
                $valid_d=mysql_query($detalle,$conex); 
                if($valid_d) 
                {
                    $m_det=@mysql_fetch_array($valid_d);
                    ...
                    {
                        $suma=$suma+$m_det['monto_abono']; 
                        if ($matriz_volados['fecha_inicio']==$m_det['fecha_m']) {
                            $cant=$cant - 1;
                        }
                        if ($m_det['monto_abono']!=0) 
                        {
                    ...
                    $suma_abo="select SUM(monto_abono) from detalle_movimientos where num_prestamo=$p_v;";
                    $resul_sum=mysql_query($suma_abo,$conex);
                    $abonado=mysql_result($resul_sum,0);
                    if ($abonado==0) 
                    {
                        $ultimo=$matriz_volados['fecha_inicio'];       
                    }
                    $cuotas=$suma/$matriz_volados['cuota'];
                    $modulo=$suma%$matriz_volados['cuota'];
                    $mod=$modulo/$matriz_volados['cuota'];
                    $cuotas=$cuotas-$mod;
                    $atr=$cant - $cuotas;
                    if ($atr<=0) 
                    {
                ...
                $pendientes=$$matriz_volados['num_cuotas']-$cuotas;
                $monto_p=$matriz_volados['monto_final']-$suma;
                $t_cartera=$t_cartera+$monto_p;
                $total_abo=$total_abo+$suma;
                $total_p=$total_p+$monto_p;
                $consul_fe="select DATE_ADD(NOW(), INTERVAL $pendientes DAY)"; 
                ...
                $f=$matriz_volados['fecha_inicio'];
                $fecha_fin=$matriz_volados['fecha_fin'];
                $consul_mora="select DATEDIFF('$fin_real','$fecha_fin')";
                $r_conmora=mysql_query($consul_mora,$conex);
                $mora=@mysql_result($r_conmora,0);          
                $actual=date("Y-m-d");
                ...
                if ($matriz_volados['fecha_inicio']==date("Y-m-d") or $cant==0) 
                {
                    $fpdf->SetFont('Arial', 'B', 8);
                    $notas="Nuevo";
                }
                elseif ($abonado>=$matriz_volados['monto_final']) 
                {  
                    $fpdf->SetFont('Arial', 'B', 8);
                    $notas="Termino";
                }
                ...
                if ($matriz_volados['dia_abona']=="Diario") 
                {
                    $dia_abona=" ";
                }
                else
                {
                    $dia_abona=$matriz_volados['dia_abona'];
                }
                $fpdf->Cell(14, 4, $notas, 1);//NOTAS
                $fpdf->Cell(6, 4, $rx, 1);//RUTA
                $fpdf->Cell(37, 4, $matriz_volados['nombre'], 1);//NOMBRE
                $fpdf->Cell(31, 4, $matriz_volados['profesion'], 1, 0, 'R');//OCUPACION
                //$number=number_format($matriz['cuota'], 0, ",",".");
                $cu=$matriz_volados['cuota']/1000;
                if ($cu<1) 
                {
                    $cu=0;
                }
                $w=$cu." x ".$matriz_volados['num_cuotas'];
                $fpdf->Cell(13, 4, $w, 1, 0, 'C');//CUOTA
                $fpdf->Cell(10, 4, $dia_abona, 1);//Dia abono
                $fpdf->Cell(7, 4, $cuotas, 1, 0, 'R');//CUOTAS ABONADAS
                $fpdf->Cell(12, 4, '  ', 1);//ABONO
                ...
                $fpdf->Cell(7, 4, $modulo, 1);//PICO
                //$number=number_format($matriz['monto'], 0, ",",".");
                $number=$matriz_volados['monto']/1000;
                $fpdf->Cell(10, 4, $number, 1);//PRESTAMO
                //$number=number_format($suma, 0, ",",".");
                $number=$suma/1000;
                $fpdf->Cell(9, 4, $number, 1);//MONTO ABONADO
                ...
                $fpdf->Cell(13, 4, $ultimo, 1, 0, 'C');//ULTIMO ABONO
                $fpdf->Cell(13, 4, $f, 1);//FECHA PRESTAMO
                $ncuotas=$matriz_volados['num_cuotas'];
                $FechaArreglo = explode("-", date("m-d-Y", strtotime($fecha_fin)));
                $Fecha = mktime(0, 0, 0, $FechaArreglo[0], $FechaArreglo[1], $FechaArreglo[2]);
                $fecha_fin=strftime("%b-%d", $Fecha);
                //fpdf->Cell(13, 4, $fecha_fin, 1);FECHA A CANCELAR
                $fpdf->Cell(30, 4, $matriz_volados['direccion'], 1);//DIRECCION
                $fpdf->Cell(25, 4, $matriz_volados['barrio'], 1);//BARRIO
                $fpdf->Cell(15, 4, $matriz_volados['ciudad'], 1);//CIUDAD
                $fpdf->Cell(19, 4, $matriz_volados['tel_fijo'], 1);//TELEFONO 1
                $fpdf->Cell(19, 4, $matriz_volados['celular'], 1);//TELEFONO 2
                $fpdf->Ln();
                $promedio=$promedio + $matriz_volados['cuota'];
                $rx=$rx+1;
            }while($matriz_volados=@mysql_fetch_array($resultado_v));
            $fpdf->Ln();
            $fpdf->SetFont('Arial', 'B', 8);
            $fpdf->Cell(14, 4, ' ', 1);//NOTAS
            $fpdf->Cell(6, 4, '', 1);//RUTA
            ...
            $fpdf->Cell(23, 4, $promedio, 1, 0, 'C');//PROMEDIO DE CUOTAS
            $fpdf->Cell(7, 4, '', 1);//# C
            $fpdf->Cell(38, 4, 'SUMA VOLADOS', 1, 0, 'C');//TOTAL CARTERA
            $t_cartera=number_format($t_cartera, 0, ",",".");
            $fpdf->Cell(19, 4, $t_cartera, 1, 0, 'C');//TOTAL CARTERA
        }
        $fpdf->Output();
